...
function squareToCoords(square, boardWidth) {
  const files = ["a","b","c","d","e","f","g","h"];
  const size = boardWidth / 8;
  return {
    x: files.indexOf(square[0]) * size + size / 2,
    y: (8 - parseInt(square[1])) * size + size / 2,
  };
}

function ArrowOverlay({ bestMove, boardWidth }) {
  ...
  useEffect(() => {
    ...
    const from = squareToCoords(bestMove.slice(0, 2), boardWidth);
    const to = squareToCoords(bestMove.slice(2, 4), boardWidth);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
        <button style={btnStyle(mode === "play", false)} onClick={() => { setMode("play"); setSelectedPiece(null); }}>▶ Play Mode</button>
        <button style={btnStyle(mode === "edit", false)} onClick={() => { setMode("edit"); setBoardPosition(fenToBoard(game.fen())); setBestMove(null); }}>✏️ Edit Mode</button>
      </div>
      ...
      <div style={{ display: "flex", gap: "20px", alignItems: "flex-start" }}>
        ...
        {/* Black Palette LEFT — fixed width */}
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <div style={{ position: "relative", width: BOARD_WIDTH, height: BOARD_WIDTH }}>
            <Chessboard
              id="chess-board"
              position={mode === "edit" ? boardPosition : game.fen()}
              onPieceDrop={mode === "edit" ? onEditDrop : onPlayDrop}
            ...
            />
            {mode === "play" && bestMove && (
              <ArrowOverlay bestMove={bestMove} boardWidth={BOARD_WIDTH} />
            )}
          </div>
        ...
        {/* White Palette RIGHT — fixed width */}
        <div style={{ minWidth: "72px" }}>
          {mode === "edit" && (
          ...
          <div style={rightPanel}>
            ...
            {gameOver && (
              <div style={{
                ...
                <p style={{
                  ...
                  color: gameOver.includes("Checkmate") ? "#e94560" : "#00d4aa"
                }}>{gameOver}</p>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                <div onClick={() => setAutoMove(!autoMove)} style={{
                  ...
                  <div style={{
                    ...
                    left: autoMove ? "25px" : "3px",
                    width: "20px", height: "20px", borderRadius: "50%",
                    background: "white", transition: "left 0.2s"
                  }} />
                </div>
                <span style={{ fontSize: "12px", color: autoMove ? "#00d4aa" : "#666" }}>
                  {autoMove ? "ON" : "OFF"}
                </span>
              </div>
            </div>
            ...
            <div style={{ background: "#16213e", padding: "14px", borderRadius: "10px", textAlign: "center" }}>
              {bestMove && !loading && (
                <>
                  ...
                  <p style={{ color: "#00d4aa", fontSize: "1.4rem", fontWeight: "bold" }}>
            ...
            <div style={{ background: "#16213e", borderRadius: "10px", padding: "14px" }}>
              ...
              <div ref={moveListRef} style={{
                ...
                background: "#0d1f1a", borderRadius: "6px", padding: "6px"
              }}>
                {moveList.length === 0
            ...
            <div style={{ background: "#16213e", borderRadius: "10px", padding: "14px" }}>
              ...
              <textarea readOnly value={game.fen()} rows={4} style={{
                width: "100%", padding: "6px", borderRadius: "6px",
                border: "1px solid #2a6b5a", background: "#0d1f1a",
                color: "#00d4aa", fontSize: "11px", resize: "none",
                fontFamily: "monospace", boxSizing: "border-box"
              }} />
              <button onClick={copyFen} style={{
                marginTop: "8px", width: "100%", padding: "8px",
                background: copied ? "#00d4aa" : "#0f3460",
                color: copied ? "#1a1a2e" : "white",
                border: "none", borderRadius: "8px", cursor: "pointer",
                fontSize: "13px", fontWeight: copied ? "bold" : "normal",
                transition: "all 0.2s"
              }}>
            ...
            {/* Reset */}
            <button onClick={resetGame} style={{
              padding: "12px", fontSize: "15px", cursor: "pointer",
              background: "#e94560", color: "white",
          ...
          <div style={rightPanel}>
            ...
            <div style={{ background: "#16213e", borderRadius: "10px", padding: "14px", textAlign: "center" }}>
              <p style={{ fontSize: "13px", color: selectedPiece ? "#00d4aa" : "#aaa" }}>
                {selectedPiece
                  ? "✅ Click any square to place piece"
            ...
            {/* Current FEN in edit mode */}
            <div style={{ background: "#16213e", borderRadius: "10px", padding: "14px" }}>
              <p style={{ fontSize: "13px", color: "#aaa", marginBottom: "8px" }}>Current FEN</p>
              <textarea readOnly value={boardToFen(boardPosition, turn)} rows={4} style={{
                width: "100%", padding: "6px", borderRadius: "6px",
                border: "1px solid #2a6b5a", background: "#0d1f1a",
                color: "#00d4aa", fontSize: "11px", resize: "none",
                fontFamily: "monospace", boxSizing: "border-box"
              ...
              <button onClick={() => {
                navigator.clipboard.writeText(boardToFen(boardPosition, turn));
                setCopied(true);
                setTimeout(() => setCopied(false), 2000);
              }} style={{
                marginTop: "8px", width: "100%", padding: "8px",
            ...
            {/* Done Editing */}
            <button onClick={applyEditPosition} style={{
